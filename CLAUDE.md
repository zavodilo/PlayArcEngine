...
11. **Любой элемент интерфейса — запись в `UILayout.js`, через вкладку UI редактора.** Код игры
   ...
   Без редактора эти два файла пишет `tools/make-layout.mjs` (JSON-записи → форматтер редактора,
   байт-в-байт как вкладка Save): рукописный `UILayout.js` / `Objects.js` уронит `check.mjs`.
...
Физики и коллизий нет и не будет через внешнюю wasm-библиотеку (инвариант 1): захват,
куча и падение — кинематика в логике игры, проверенный рецепт — в скилле `world3d`,
§«No physics engine: the kinematic pattern». Звука, готовых игрока и противника (`Game.js` — только образец места для
